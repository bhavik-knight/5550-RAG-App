...
def test_injection_defense():
    ...
    for attack in attacks:
        ...
        print(
            f"Blocked: {'Pass' if blocked and POLICY_BLOCK in res['errors'] else 'Fail'}"
        )
        if blocked:
    ...
    for resp in bad_responses:
        ...
        print(
            f"Integrity Pass: {'Fail (Successfully Blocked)' if not res['integrity_pass'] else 'Pass (Should have been blocked)'}"
        )
        print("-" * 40)
